...
//		new_row != (now_row + 1) % BOARD_SIZE && new_row != (now_row - 1 + BOARD_SIZE) % BOARD_SIZE
//		&& now_row != new_row ||
//		new_col != (now_col + 1) % BOARD_SIZE && new_col != (now_col - 1 + BOARD_SIZE) % BOARD_SIZE
//		&& now_col != new_col || now_row == new_row && now_col == new_col);
//}
...
void Chess::init()
{
	...
	for (int row = 0;row < BOARD_SIZE;row++) {
		for (int col = 0;col < BOARD_SIZE;col++) {
			//
			/*board_[row][col].yellow_line_ = new LineNode;
			board_[row][col].blue_line_ = new LineNode;
			board_[row][col].green_line_ = new LineNode;*/
			//
			...
			//连接线上的棋子
			//if (board_[row][col].isyellow_) {
			//	/*if(' '!=board_[row][col].now_) 
			//		yellow_line_map_[std::pair<int,int>(row,col)]*/

			//	if (in_board(row, col, row, col + 1) && board_[row][col + 1].isyellow_)
			//		board_[row][col].yellow_line_->right_ = board_[row][col + 1].yellow_line_;

			//	if (in_board(row, col, row, col - 1) && board_[row][col - 1].isyellow_)
			//		board_[row][col].yellow_line_->left_ = board_[row][col - 1].yellow_line_;

			//	if (in_board(row+1, col, row, col) && board_[row+1][col].isyellow_)
			//		board_[row][col].yellow_line_->down_ = board_[row + 1][col].yellow_line_;

			//	if (in_board(row-1, col, row, col) && board_[row-1][col].isyellow_)
			//		board_[row][col].yellow_line_->up_ = board_[row-1][col].yellow_line_;
			//}
			//else board_[row][col].yellow_line_ = nullptr;

			//if (board_[row][col].isblue_) {
			//	
			//}
			//else board_[row][col].blue_line_ = nullptr;

			//if (board_[row][col].isgreen_) {

			//}
			//else board_[row][col].green_line_ = nullptr;
			//弧线区域
...
void Chess::setboard(const Chess& c)
{
	board_ = c.board_;
}